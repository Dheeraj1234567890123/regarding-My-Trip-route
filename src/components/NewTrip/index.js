...
const stepsList = [
  ...
  {stepId: 'GUESTS', displayText: 'Guests', completed: false, stepNumber: 3},
  {
...
class NewTrip extends Component {
  ...
  onSubmitform = (event, addTrip) => {
    ...
    const updatedStepList = initialStepList.map(eachStep => {
      if (eachStep.stepId === activeId) {
        return {
          ...eachStep,
          completed: true,
        }
    ...
    this.setState({
      ...
      tripData: newTrip,
      initialStepList: updatedStepList,
    })
    addTrip({...tripData})
  }
  ...
  onClickYourDetails = activeId => {
    const {nameINput, startLocationInput, endLocationInput, initialStepList} =
      this.state
    const updatedStepList = initialStepList.map(eachStep => {
      if (eachStep.stepId === activeId) {
        return {
    ...
    return (
      <>
        ...
        <button
          ...
          type="button"
          onClick={() => this.onClickYourDetails(activeId)}
        >
          Next
        </button>
      </>
  ...
  onClickDateButton = activeId => {
    const {startDateInput, endDateInput, initialStepList} = this.state
    const updatedStepList = initialStepList.map(eachStep => {
      if (eachStep.stepId === activeId) {
        return {
          ...eachStep,
    ...
    if (
      ...
      this.setState({
        activeId: stepsList[2].stepId,
        initialStepList: updatedStepList,
      })
    }
  }
  onClickPreviousDateButton = activeId => {
    const {initialStepList} = this.state
    const updatedStepList = initialStepList.map(eachStep => {
      if (eachStep.stepId === activeId) {
        return {
          ...eachStep,
      ...
      <>
        ...
        <div className="buttons-container">
          <button
            ...
            onClick={() => this.onClickPreviousDateButton(activeId)}
          >
            Previous
          </button>
          <button
            ...
            onClick={() => this.onClickDateButton(activeId)}
          >
            Next
          </button>
        </div>
  ...
  onClickPreviousGuestsButton = activeId => {
    const {initialStepList} = this.state
    const updatedStepList = initialStepList.map(eachStep => {
      if (eachStep.stepId === activeId) {
        return {
  ...
  onClickGuestsButton = activeId => {
    ...
    this.setState({
      activeId: stepsList[3].stepId,
      initialStepList: updatedStepList,
    })
  }

  renderGuests = () => {
    const {adultCount, childCount, infantsCount, activeId} = this.state
    return (
      <ol className="ordered-list-container">
        ...
        <div className="buttons-container">
          <button
            type="button"
            className="previous-button"
            onClick={() => this.onClickPreviousGuestsButton(activeId)}
          >
            Previous
          </button>
          <button
            type="button"
            className="next-button"
            onClick={() => this.onClickGuestsButton(activeId)}
          >
            Next
          </button>
        </div>
  ...
  onClickPreviousTravrlButton = activeId => {
    const {initialStepList} = this.state
    const updatedStepList = initialStepList.map(eachStep => {
      if (eachStep.stepId === activeId) {
        return {
  ...
  onClickTravelButton = activeId => {
    ...
    this.setState({
      activeId: stepsList[4].stepId,
      initialStepList: updatedStepList,
    })
  }

  renderTravelAssistance = () => {
    const {selectId, checkInput, activeId} = this.state
      ...
      <div className="travel-container">
        ...
        <div className="buttons-container">
          <button
            type="button"
            className="previous-button"
            onClick={() => this.onClickPreviousTravrlButton(activeId)}
          >
            Previous
          </button>
          <button
            type="button"
            className="next-button"
            onClick={() => this.onClickTravelButton(activeId)}
          >
            Next
          </button>
        </div>
  ...
  onClickPreviousConfirmationButton = activeId => {
    const {initialStepList} = this.state
    const updatedStepList = initialStepList.map(eachStep => {
      if (eachStep.stepId === activeId) {
        return {
  ...
  onClickNextTrip = () => {
    const {initialStepList, activeId} = this.state
    const updatedStepList = initialStepList.map(eachStep => {
      return {
        ...eachStep,
        completed: false,
        ...
        <div className="buttons-container">
          <Link to="/book-a-new-trip" className="link">
            <button
              type="button"
              className="previous-button"
              onClick={() => this.onClickPreviousConfirmationButton(activeId)}
            >
              Cancel
            </button>
          </Link>
  ...
  render() {
    const {activeId, confirmationPage, tripData, initialStepList} = this.state
                ...
                <div className="card-container">
                  <div className="details-container">
                    <div className="step-list-container">
                      {initialStepList.map((eachStep, index) => (
                    ...
                    <div className="your-details-container">
                      ...
                      <div className="card">
                        <form
                          className="forms"
                          onSubmit={event => this.onSubmitform(event, addTrip)}
                        >
